...
class PlantingSeasonAdapter(
    ...
    private val plantingSeasonAdapterCallBack: PlantingSeasonAdapter.PlantingSeasonAdapterCallBack
) : RecyclerView.Adapter<PlantingSeasonAdapter.ViewHolder>(), MonthsAdapter.MonthsAdapterCallBack{

    interface PlantingSeasonAdapterCallBack {
        fun onPlantingMonthSelected(cropResponse: WgCropProductionResponseItem, selectedMonth: MonthsModel)
    }
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        if(currentCropResponseItem.plantingPeriod.isNotEmpty()) {
            viewHolder.selectMonths.text = returnMonthInitialsString(currentCropResponseItem.plantingPeriod)
        } else {
        ...
        viewHolder.selectMonths.setOnClickListener {
            inflateSeasonCalendarModal(
                months,
                SeasonsResponsesEnum.DYNAMIC_PLANTING,
    ...
    override fun onMonthSelected(
        ...
        cropResponseItem?.let {
            plantingSeasonAdapterCallBack.onPlantingMonthSelected(
                it,selectedMonth)
        }
    }
